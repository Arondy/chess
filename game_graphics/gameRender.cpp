...
GameRender::GameRender() : window{VideoMode(windowXStartingSize, windowXStartingSize / aspectRatio), "MyChess"},
    ...
    Image icon;
    if (!icon.loadFromFile("../game_graphics/textures/icon.png")){
        throw;
    }
...
void GameRender::resize_window(Vector2u &prevWinSize){
    ...
    cout << "Resized window size: " << newWinSize.x << "x" << newWinSize.y << endl;
}

void GameRender::load_textures(){
    if (!textures["board"][0].loadFromFile("../game_graphics/textures/board.png")){
        throw;
    }
    if (!textures["shapes"][0].loadFromFile("../game_graphics/textures/circle.png")){
        throw;
    }
    if (!textures["shapes"][1].loadFromFile("../game_graphics/textures/rectangle.png")){
        throw;
    }
    if (!textures["shapes"][2].loadFromFile("../game_graphics/textures/border.png")){
        throw;
    }
    if (!textures["shapes"][3].loadFromFile("../game_graphics/textures/rounded_rectangle.png")){
        throw;
    }
    if (!textures["shapes"][4].loadFromFile("../game_graphics/textures/border_red.png")){
        throw;
    }
    ...
    for (int i = 0; i < figureNum * textureSize; i += textureSize){
        ...
        if (!texture.loadFromFile("../game_graphics/textures/figures.png",
...
void GameRender::load_font(){
    if (!fonts["clock"].loadFromFile("../game_graphics/fonts/calibri_bold.ttf")){
        throw;
    }
...
void GameRender::place_sprite(Sprite &sprite, int i, int j){
    Vector2u winSize = window.getSize();
    Vector2f position;
    position.x = offsetRatio * winSize.x + j * squareSize;
